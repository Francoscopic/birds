...
class SigninProcessor extends AbstractController
{
        ...
        if( isset($_POST['clt'], $_POST['psw']) ) {
            ...
            if($this->validate_user_input($user, $pass) === true) {
                ...
                if($this->validate_details_exist($user) === true) {
                    ...
                    $get = $this->validate_correct_details($user, $pass)['handle'];
                    if($get == true) {
    ...
    protected function validate_correct_details($email, $pass): array 
    {
        ...
        $row = $result->fetch_array(MYSQLI_ASSOC);
            ...
            $uid = $row['uid'];
            if(validate_user_access($uid) != true) {
                # Exceeded login trial
                echo 80;
                $handle = false;
            } elseif (validate_password($email, $pass) != true) {
                # password incorrect
                echo_error('Email/Password incorrect');
                update_login_passcount($uid);   # Do security.
                $handle = false;
            }
